...
def lifecycle_status_embed(state: str, description: str, *, channel_role: str = "main") -> EmbedPayload:
    ...
    if channel_role == "admin":
        ...
        field_value = (
            ...
            "`u!fixpreviews` - repair proof previews\n"
            "`u!clean` - clean channel clutter"
        )
    else:
...
def lifecycle_stats_embed(entries: list[LeaderboardEntry]) -> EmbedPayload:
    ...
    top_lines = (
        ...
        if not entries
            ...
            f"{index}. <@{entry.user_id}> | Sales ${format_money(entry.total_sales)} | Logs {entry.receipt_count}"
    ...
    return (
        ...
        .field("Logs", str(total_receipts), True)
...
def render_stats_description(entries: list[LeaderboardEntry]) -> str:
    if not entries:
        return "No logs counted in stats yet."
    return "\n".join(
        (
            f"{index}. <@{entry.user_id}> | Total Sales ${format_money(entry.total_sales)} | "
            f"Company Cost ${format_money(entry.procurement_cost)} | Logs {entry.receipt_count}"
        )
...
def render_payout_description(entries: list[PayoutEntry]) -> str:
    if not entries:
        return "No unpaid logs or remits waiting for payout."
    lines: list[str] = []
    for index, entry in enumerate(entries, start=1):
...
def help_page_embed(prefix: str, page: int) -> EmbedPayload:
    page = _clamp_help_page(page)
    embed = panel_embed(
        f"UWU Cafe Help - {_help_page_title(page)}",
    ...
    if page == 0:
        embed.field(
            "Everyday Commands",
            (
                f"`{prefix}log <count>` - Log combo sales with an attached proof image\n"
                f"`{prefix}remit <item> <amount>` - Remit materials with an attached proof image\n"
                f"`{prefix}stats` / `/mechstats` - Show the leaderboard for 1 minute\n"
                f"`{prefix}note <receipt_id> <note>` / `/mechnote` - Add or update a log note\n"
            ...
            "Where To Use Them",
            "`log` belongs in the configured log forum threads. `remit` belongs in the configured remit channel.",
            False,
        )
    elif page == 1:
        embed.field(
            ...
            (
                ...
                f"`{prefix}weeklypayout` / `/mechweeklypayout` - Freeze the week and award bonuses\n"
                ...
                f"`{prefix}reset [user]` / `/mechreset` - Backup active logs, then mark them paid or invalidate them\n"
        ...
        embed.field(
            ...
            "Active logs count in payouts and stats.\nPaid logs stay in stats only.\nInvalidated logs are excluded.",
            False,
        )
    else:
        embed.field(
...
def stats_embed(sort: StatsSort, entries: list[LeaderboardEntry]) -> EmbedPayload:
    ...
    total_receipts = sum(entry.receipt_count for entry in entries)
    embed = (
        panel_embed("UWU Cafe Stats", render_stats_description(entries))
    ...
    if entries:
        return embed.with_footer(
            "Showing active and paid logs. Invalidated logs are excluded."
        )
    return embed.with_footer("No logs counted in stats yet.")
...
def payouts_embed(entries: list[PayoutEntry]) -> EmbedPayload:
    ...
    embed = (
        panel_embed("UWU Cafe Payouts", render_payout_description(entries))
        .field("Employees", str(len(entries)), True)
    ...
    if entries:
        return embed.with_footer("Showing unpaid logs and open remit adjustments only.")
    return embed.with_footer("No unpaid logs or remits waiting for payout.")


def pricesheet_embed(items: list[CatalogItem]) -> EmbedPayload:
    return panel_embed(
        "UWU Cafe Price Sheet",
...
def contracts_embed(contracts: list[Contract]) -> EmbedPayload:
    ...
    ).with_footer("Contracts are retained only for import compatibility.")
...
def health_embed(
    ...
    embed = (
        panel_embed("UWU Cafe Health", f"Status: {status}")
        if not issues
        else warning_panel_embed("UWU Cafe Health", f"Status: {status}")
    )
    embed.field("Checks", "\n".join(checks), False)
...
def calc_embed(catalog_items: list[CatalogItem], session) -> EmbedPayload:
    embed = panel_embed("UWU Cafe Legacy Calculator", render_session_description(catalog_items, session))
    ...
    return embed.with_footer(
        "Legacy calculator flow is disabled for UWU Cafe. Use text logs with proof images."
    )
...
def calc_completed_embed(receipt_id: str) -> EmbedPayload:
    return panel_embed(
        "UWU Cafe Log",
        (
            f"Log Saved: `{receipt_id}`.\n"
            "You may view it to edit or invalidate this log.\n"
            "This message will be deleted in 10 seconds."
        ),
    ).with_footer("Log saved.")
...
def calc_embed_for_status(
    ...
    embed = EmbedPayload(
        title="UWU Cafe Legacy Calculator",
...
def render_session_description(
    ...
    *,
    ...
    return (
        ...
        f"**Commission:** ${format_money_cents(staff_pay_cents)}\n"
        f"**Proof:** {session_proof_status(session)}\n"
        f"{workflow_notice}"
        f"**Billable Items:**\n{billable_text}\n\n"
...
def manage_page_parts(
    ...
    embed = (
        panel_embed("UWU Cafe Log Manager", description)
        .field("Page", str(page + 1), True)
        .with_footer("Select a log number below to inspect or change status.")
    )
    return embed, rows
...
def receipt_detail_payload(
    ...
    base_embed = panel_embed(f"UWU Cafe Log {receipt.id}", description)
    base_embed.field(
        "Credited To",
        ...
        "Active logs affect payouts. Paid logs stay in stats only. Invalidated logs are excluded. Replace Proof keeps the log ID."
    )
...
def receipt_message_content(receipt_id: str, creator_user_id: str) -> str:
    return f"Log `{receipt_id}` saved for <@{creator_user_id}>."
...
def receipt_embed(
    ...
    embed = EmbedPayload(
        title=f"UWU Cafe Log {receipt_id}",
        description=description,
        color=THEME_SUCCESS,
    )
    if admin_note:
