...
from .command_support import format_template_command_output, parse_signed_amount_cents_text, parse_user_token
from .render import (
    ReplyPayload,
    health_embed,
    help_action_rows,
...
async def _ensure_allowed_channel(ctx: CommandContext) -> None:
    ...
    result = CommandResult(
        canonical_name=None,
        events=[_send(_error_reply(ctx, "UWU Cafe Access", "This command is not enabled in this channel."))],
    )
...
async def _ensure_admin(ctx: CommandContext) -> None:
    ...
    result = CommandResult(
        canonical_name=None,
        events=[_send(_error_reply(ctx, "UWU Cafe Access", "You are not an admin for this bot."))],
    )
    result.events.extend(_schedule_transient_cleanup_events(ctx))
    raise _handled(result)
...
async def _ensure_admin_channel(ctx: CommandContext) -> None:
    ...
    result = CommandResult(
        canonical_name=None,
        events=[_send(_error_reply(ctx, "UWU Cafe Access", "This admin command is not enabled in this channel."))],
    )
    result.events.extend(_schedule_transient_cleanup_events(ctx))
    raise _handled(result)
...
async def _handle_log_attachment_only(ctx: CommandContext, args: list[str]) -> CommandResult:
    del args
    result = CommandResult(
        canonical_name="log",
        events=[
            _send(
                _error_reply(
                    ctx,
                    "UWU Cafe Log",
                    "Use this in Discord as `u!log <combo_count>` with an attached proof image.",
                )
            )
        ],
    )
...
async def _handle_remit_attachment_only(ctx: CommandContext, args: list[str]) -> CommandResult:
    del args
    result = CommandResult(
        canonical_name="remit",
        events=[
            _send(
                _error_reply(
                    ctx,
                    "UWU Cafe Remit",
                    "Use this in Discord as `u!remit <item> <amount>` with an attached proof image.",
                )
            )
        ],
    )
    ...
    try:
        ...
        raise _handled(
            CommandResult(
                ...
                events=[_send(_error_reply(ctx, "UWU Cafe Payout Split", str(error)))],
            )
        ) from error
...
async def _handle_weeklypayout(ctx: CommandContext, args: list[str]) -> CommandResult:
    ...
    if top_entry is not None:
        winners.append(
            (
                ...
                f"Top Seller bonus for {week_key}",
            )
        )
    ...
    description = (
        ...
        f"Top Seller: {_weekly_winner_line(top_entry, WEEKLY_TOP_MECH_BONUS_CENTS)}\n"
    ...
    events.append(
        _send(
            ReplyPayload(
                embeds=[task_status_embed("UWU Cafe Weekly Payout", description)],
                ephemeral=ctx.is_interaction,
            )
        )
    )
...
async def _handle_refresh(ctx: CommandContext, args: list[str]) -> CommandResult:
    ...
    events = [
        _send(
            ReplyPayload(
                embeds=[task_status_embed("UWU Cafe Refresh", "Reloading catalog and package files from disk now...")],
                ephemeral=ctx.is_interaction,
            )
        )
    ]
...
async def _handle_templates(ctx: CommandContext, args: list[str]) -> CommandResult:
    ...
    if action == "reload":
        ...
        if report.status is TemplateLoadStatus.LOADED:
            ...
            embed = task_status_embed("UWU Cafe Templates", description)
            ...
            embed = task_warning_embed("UWU Cafe Templates", description)
        events = _cleanup_prefix_invocation_events(ctx)
    ...
    template_text = format_template_command_output(templates) if templates is not None else "No templates loaded."
    reply = ReplyPayload(
        content=template_text,
        embeds=[
            task_status_embed(
                "UWU Cafe Templates",
                "Templates are currently blank until UWU Cafe announcement copy is added.",
            )
        ],
...
async def _handle_health(ctx: CommandContext, args: list[str]) -> CommandResult:
    ...
    if callable(report_builder):
        ...
        if not [channel_id for channel_id in config.allowed_channel_ids if channel_id > 0]:
            ...
            checks.append(f"Configured main channels: {len([channel_id for channel_id in config.allowed_channel_ids if channel_id > 0])}")
        if not [channel_id for channel_id in config.admin_channel_ids if channel_id > 0]:
            ...
            checks.append(f"Configured admin channels: {len([channel_id for channel_id in config.admin_channel_ids if channel_id > 0])}")
        if config.remit_channel_id is None:
            issues.append("`remit_channel_id` is not set. Material remits will not be accepted.")
        else:
            checks.append(f"Remit channel configured: <#{config.remit_channel_id}>")
...
async def _handle_export(ctx: CommandContext, args: list[str]) -> CommandResult:
    ...
    events = [
        _send(
            ReplyPayload(
                embeds=[task_status_embed("UWU Cafe Export", "Exporting the current database now...")],
                ephemeral=ctx.is_interaction,
            )
        )
    ]
    bundle = await ctx.runtime.database.export_bundle(ctx.runtime.catalog)
    path = await save_export_async(bundle, ctx.runtime.config.storage.export_dir, "uwu-cafe-export")
    events.append(
        _edit(
            ReplyPayload(
                embeds=[
    ...
    if parts[:2] != ["help", "page"] or len(parts) != 4:
        return CommandResult(
            ...
            events=[
                _send(
                    ReplyPayload(
                        embeds=[task_error_embed("UWU Cafe Help", f"Unknown component `{custom_id}`.")],
                        ephemeral=ctx.is_interaction,
                    )
                )
            ],
    ...
    if owner_user_id != ctx.actor.user_id:
        return CommandResult(
            ...
            events=[
                _send(
                    ReplyPayload(
                        embeds=[task_error_embed("UWU Cafe Help", "This help panel belongs to another user.")],
                        ephemeral=ctx.is_interaction,
                    )
                )
            ],
...
COMMAND_HANDLERS: dict[str, CommandHandler] = {
    "log": _handle_log_attachment_only,
    "remit": _handle_remit_attachment_only,
    "help": _handle_help,
...
def _parse_payout_split_args(args: list[str]) -> tuple[str, list[str], bool, str]:
    if len(args) < 2:
        raise ValueError("Use `u!payoutsplit @source everyone|@user... [reason]`.")
