...
class DiscordConfig(BaseModel):
    ...
    remit_channel_id: int | None = None
    ...
    @field_validator("remit_channel_id", "receipt_log_channel_id", mode="before")
    @classmethod
    def normalize_optional_channel_id(cls, value: object) -> object:
        if value in {None, "", 0, "0"}:
            return None
        return value
...
class AppConfig(BaseModel):
    ...
    def resolve_token(self, env: dict[str, str], *, require_token: bool = True) -> None:
        env_token = (
            env.get("UWU_CAFE_DISCORD_TOKEN", "").strip()
