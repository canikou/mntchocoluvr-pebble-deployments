...
LOCAL_STOP_SIGNAL_FILE = "uwu-cafe.stop"
...
class ConsoleRuntime:
    ...
    async def run(self) -> int:
        print("UWU Cafe local parity console")
    ...
    async def _seed_demo_data(self) -> None:
        ...
        active = NewReceipt(
            id=f"UWU-DEMO-ACTIVE-{suffix}",
            creator_user_id="42",
            creator_username="42",
            creator_display_name="Tester",
            guild_id="demo",
            channel_id=str(self.state.channel.channel_id),
            total_sale=COMBO_UNIT_PRICE * 2,
            procurement_cost=0,
            profit=COMBO_UNIT_PRICE * 2,
            ...
            items=[
                PricedItem(
                    ...
                    line_sale_total=COMBO_UNIT_PRICE * 2,
                    line_cost_total=0,
                )
            ],
        )
        paid = NewReceipt(
            id=f"UWU-DEMO-PAID-{suffix}",
            creator_user_id="42",
            creator_username="42",
            creator_display_name="Tester",
            guild_id="demo",
            channel_id=str(self.state.channel.channel_id),
            total_sale=COMBO_UNIT_PRICE,
            procurement_cost=0,
            profit=COMBO_UNIT_PRICE,
            ...
            items=[
                PricedItem(
                    ...
                    unit_cost=COMBO_UNIT_COST,
                    pricing_source=PricingSource.DEFAULT,
                    line_sale_total=COMBO_UNIT_PRICE,
                    line_cost_total=0,
                )
            ],
        )
        invalidated = NewReceipt(
            id=f"UWU-DEMO-VOID-{suffix}",
            creator_user_id="7",
            creator_username="7",
...
class BakunawaMechDiscordClient(discord.Client):
        ...
        if interaction.channel is not None and not self._is_scoped_messageable_channel(interaction.channel):
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "UWU Cafe is only enabled in its configured shop channels.",
                    ephemeral=True,
                )
            return
        if (
            ...
            try:
                ...
                await _safe_interaction_warning(
                    ...
                    (
                        ...
                        "The bot logged the stack trace to `logs/uwu-cafe.log`."
                    ),
                )
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if prefix_match is not None:
            ...
            if command_name == "log":
                await self._run_log_prefix(message, remainder)
                return
            if command_name == "remit":
                await self._run_remit_prefix(message, remainder)
                return
            if command_name == "calc":
                await self._send_legacy_calc_disabled(message)
                return
    ...
    async def _announce_starting_up(self) -> None:
        ...
        self._log_status = LifecycleStatusState(
            kind=LifecycleStatusKind.STARTING,
            description="UWU Cafe log tracking is starting up. Status and stats will refresh shortly.",
        )
        self._main_status = LifecycleStatusState(
            kind=LifecycleStatusKind.STARTING,
            description="The UWU Cafe bot is starting up. Commands will be available in a moment.",
        )
    ...
    async def _announce_ready(self) -> None:
        ...
        self._admin_status = LifecycleStatusState(
            ...
            description=(
                "Bot is fully online, ready for UWU Cafe logs.\n\n"
                f"{prefix}payouts to see current staff payouts. "
                f"{prefix}reset to mark active logs paid or invalidate them."
            ),
        )
        self._log_status = LifecycleStatusState(
            kind=LifecycleStatusKind.ONLINE,
            description=(
                "Combo logs are online. Use the existing forum thread for each user.\n\n"
                f"{prefix}log <count> with a proof image records sales."
            ),
        )
        self._main_status = LifecycleStatusState(
            kind=LifecycleStatusKind.ONLINE,
            description=f"Bot is fully online for UWU Cafe.\n\nUse {prefix}log <count> with a proof image.",
        )
        await self._refresh_lifecycle_status_messages_now(force=True, respect_active_session=False)
    ...
    async def _announce_shutdown_status(
        ...
        *,
        ...
        if restart:
            if contact_user_id is not None:
                main_reason = (
                    f"The UWU Cafe bot is shutting down for a restart. "
                    f"Please let <@{contact_user_id}> know if you need help while it is offline."
                )
            else:
                main_reason = (
                    "The UWU Cafe bot is shutting down for a restart. "
                    "Please let an admin know if you need help while it is offline."
                )
        else:
            if contact_user_id is not None:
                main_reason = (
                    f"The UWU Cafe bot is shutting down. "
                    f"Please let <@{contact_user_id}> know if you need help while it is offline."
                )
            else:
                main_reason = (
                    "The UWU Cafe bot is shutting down. "
                    "Please let an admin know if you need help while it is offline."
                )
        ...
        self._log_status = LifecycleStatusState(
            kind=LifecycleStatusKind.STOPPING,
            description="UWU Cafe logging is shutting down now.",
        )
        self._main_status = LifecycleStatusState(
            kind=LifecycleStatusKind.STOPPING,
    ...
    async def _announce_stopped_status(self) -> None:
        ...
        main_reason = (
            f"The UWU Cafe bot is offline. Please let <@{contact_user_id}> know if you need help."
            if contact_user_id is not None
            else "The UWU Cafe bot is offline. Please let an admin know if you need help."
        )
        self._admin_status = LifecycleStatusState(
        ...
        self._log_status = LifecycleStatusState(
            kind=LifecycleStatusKind.STOPPED,
            description="UWU Cafe logging is offline. Start the runner again to restore logging.",
        )
        self._main_status = LifecycleStatusState(
            kind=LifecycleStatusKind.STOPPED,
            description=main_reason,
    ...
    def _is_scoped_channel_id(self, channel_id: int) -> bool:
        ...
        return (
            ...
            or channel_id == config.remit_channel_id
    ...
    def _is_remit_messageable_channel(self, channel: discord.abc.MessageableChannel) -> bool:
        remit_channel_id = self.base_runtime.config.discord.remit_channel_id
    ...
    async def _run_log_prefix(self, message: discord.Message, remainder: str) -> None:
            ...
            await message.channel.send(
                embeds=[
                    embed_from_payload(
                        task_error_embed(
                            "UWU Cafe Log",
                            "This command is only enabled in the configured log forum threads.",
                        )
                    )
                ]
            )
            ...
            await message.channel.send(
                embeds=[
                    embed_from_payload(
                        task_error_embed(
                            "UWU Cafe Log",
                            "Attach a proof image when logging combo sales.",
                        )
                    )
                ]
            )
        ...
        try:
            proof_paths, proof_urls = await self._save_message_proofs(message, receipt_id, proof_attachments)
            receipt, accounting = self._build_combo_receipt(
                message=message,
            ...
            LOGGER.exception("message_id=%s failed to save UWU Cafe log", message.id)
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Log", f"Log failed: {error}"))]
            )
            return
    ...
    async def _run_remit_prefix(self, message: discord.Message, remainder: str) -> None:
        if not self._is_remit_messageable_channel(message.channel):
            await message.channel.send(
                embeds=[
                    embed_from_payload(
                        task_error_embed(
                            "UWU Cafe Remit",
                            "This command is only enabled in the configured remit channel.",
                        )
                    )
                ]
            )
            return
        try:
            item, amount = parse_remit_args(remainder)
        except ValueError as error:
            await message.channel.send(
            ...
            await message.channel.send(
                embeds=[
                    embed_from_payload(
                        task_error_embed(
                            "UWU Cafe Remit",
                            "Attach a proof image when remitting materials.",
                        )
                    )
                ]
            )
        ...
        remit_id = f"UWU-REMIT-{message.id}"
        try:
            _proof_paths, proof_urls = await self._save_message_proofs(message, remit_id, proof_attachments)
            payout_cents = amount * item.unit_rate * 100
            reason = (
                f"Remit {item.display_name} x{amount:,} @ ${item.unit_rate:,} each"
                f" | proof {proof_urls[0] if proof_urls else 'attached'}"
            )
            inserted = await self.base_runtime.database.apply_payout_adjustments(
                [
                    (
            ...
            LOGGER.exception("message_id=%s failed to save UWU Cafe remit", message.id)
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Remit", f"Remit failed: {error}"))]
            )
            return
            ...
            embeds=[
                embed_from_payload(
                    task_status_embed(
                        "UWU Cafe Remit",
                        (
                            f"Logged {item.display_name} x{amount:,} for <@{message.author.id}>.\n"
                            f"Rate: ${item.unit_rate:,} each\n"
                            f"Payout increase: ${payout_cents // 100:,}\n"
                            f"Adjustment rows created: {inserted}"
                        ),
                    )
                )
            ],
    ...
    async def _send_legacy_calc_disabled(self, message: discord.Message) -> None:
        await message.channel.send(
            embeds=[
                embed_from_payload(
                    task_error_embed(
                        "UWU Cafe Log",
                        "The calculator flow has been replaced. Use `u!log <combo_count>` with an attached proof image.",
                    )
                )
            ]
        )

    async def _save_message_proofs(
        self,
        message: discord.Message,
        record_id: str,
        ...
        return proof_paths, proof_urls

    def _build_combo_receipt(
        self,
        *,
        message: discord.Message,
        receipt_id: str,
    ...
    async def _run_calc_prefix(self, message: discord.Message, remainder: str) -> None:
        if not self._is_allowed_messageable_channel(message.channel):
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Access", "This command is not enabled in this channel."))]
            )
            return
    ...
    async def _run_import_prefix(self, message: discord.Message, remainder: str) -> None:
        ...
        try:
            ...
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Import", str(error)))]
            )
            return
    ...
    async def _run_clean_prefix(self, message: discord.Message) -> None:
            ...
            await message.channel.send(
                embeds=[
                    embed_from_payload(
                        task_error_embed(
                            "UWU Cafe Clean",
                            "Zero-deletion policy is enabled in the admin channel. Use `u!cleanlog` for the main channel instead.",
                        )
                    )
                ]
            )
        ...
        progress = await message.channel.send(
            embeds=[embed_from_payload(task_status_embed("UWU Cafe Clean", "Cleaning non-log messages in this channel..."))]
        )
    ...
    async def _run_clean_log_prefix(self, message: discord.Message) -> None:
        ...
        if not self._is_admin_messageable_channel(message.channel):
            ...
            return
        progress = await message.channel.send(
            embeds=[embed_from_payload(task_status_embed("UWU Cafe Main Clean", "Cleaning top-level clutter in the main channel..."))]
        )
    ...
    async def _run_note_prefix(self, message: discord.Message, remainder: str) -> None:
        ...
        try:
            ...
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Receipt Note", str(error)))]
            )
            return
        receipt = await self.base_runtime.database.get_receipt(receipt_id)
        if receipt is None:
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Receipt Note", f"Receipt `{receipt_id}` was not found."))]
            )
            return
        if str(message.author.id) != receipt.creator_user_id and not self._is_admin_user(message.author.id):
            await message.channel.send(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Receipt Note", "Only the receipt creator or an admin can edit that note."))]
            )
            return
    ...
    async def _get_or_create_employee_thread(
        ...
        return await parent.create_thread(
            ...
            reason="UWU Cafe employee thread",
        )
        ...
        *,
        ...
        try:
            ...
            if interaction is not None:
                await interaction.response.send_message(
                    embed=embed_from_payload(task_error_embed("UWU Cafe Access", str(error))),
                    ephemeral=True,
                )
            elif prefix_message is not None:
                await prefix_message.channel.send(
                    embeds=[embed_from_payload(task_error_embed("UWU Cafe Access", str(error)))]
                )
            return
        ...
        if interaction is not None and interaction.channel_id != target_channel.id:
            await interaction.response.send_message(
                f"I opened your UWU Cafe calculator in {target_channel.mention}.",
                ephemeral=True,
            )
        if prefix_message is not None and target_channel.id != prefix_message.channel.id:
            referral = await prefix_message.channel.send(
                f"<@{actor.id}> I opened your UWU Cafe calculator in {target_channel.mention}."
            )
            self._schedule_delete_message_if_allowed(
                referral,
                self.base_runtime.config.discord.transient_message_timeout_seconds,
                ...
                if isinstance(panel_channel, discord.abc.Messageable):
                    ...
                    if panel_message is not None:
                        if processing.rescan_active:
                            ...
                            await panel_message.edit(
                                content="",
                                embeds=[embed_from_payload(task_status_embed("UWU Cafe Rescan", "Receipt saved. No more undocumented receipt candidates remain in this queue."))],
                                view=None,
                            )
                        else:
                            await panel_message.edit(
    ...
    async def build_health_report(self) -> tuple[list[str], list[str]]:
        ...
        if not [channel_id for channel_id in config.allowed_channel_ids if channel_id > 0]:
            issues.append("`allowed_channel_ids` is empty. Add at least one main channel ID.")

        if not [channel_id for channel_id in config.admin_channel_ids if channel_id > 0]:
            issues.append("`admin_channel_ids` is empty. Admin commands will not be usable anywhere.")

        if config.remit_channel_id is None:
            issues.append("`remit_channel_id` is not set. Material remits will not be accepted.")
        ...
        if config.remit_channel_id is not None:
            await self._inspect_channel(
                config.remit_channel_id,
                "Remit channel",
                remit_required,
                checks,
                issues,
        ...
        if action == "rescan_skip":
            ...
            if next_session is None:
                ...
                await interaction.response.edit_message(
                    content="",
                    embeds=[embed_from_payload(task_status_embed("UWU Cafe Rescan", "No more undocumented receipt candidates remain in this queue."))],
                    view=None,
                )
    ...
    async def _handle_reset_component(
        ...
        if action == "confirm":
            ...
            if pending is None:
                await interaction.response.edit_message(
                    embeds=[embed_from_payload(task_error_embed("UWU Cafe Reset", "This reset prompt expired. Run `u!reset` again."))],
                    view=None,
                )
                return
            buttons = [
            ...
            if panel is None or panel.expires_at < utcnow():
                ...
                await interaction.response.send_message(
                    embed=embed_from_payload(
                        task_warning_embed(
                            "Help Panel Expired",
                            "Run `u!help` or `/mechhelp` again to reopen the help panel.",
                        )
                    ),
                    ephemeral=True,
                )
    ...
    async def _send_reset_confirmation(
        ...
        *,
        ...
        fixed_payload = ReplyPayload(
            embeds=[task_warning_embed("UWU Cafe Reset", _reset_warning_message(mode, user_ids))],
            components=[
                ActionRowPayload(
                    components=[
                        ButtonPayload(
    ...
    async def _stage_import_review(
        ...
        *,
        ...
        confirm_payload = ReplyPayload(
            embeds=[task_warning_embed("UWU Cafe Import", _import_confirmation_message(status_override, preview))],
            components=[
                ActionRowPayload(
                    components=[
                        ButtonPayload(
    ...
    async def _start_rescan(
        ...
        *,
        ...
        if not boundary:
            ...
            if interaction is not None:
                await interaction.response.send_message(embed=embed_from_payload(task_error_embed("UWU Cafe Rescan", error_text)), ephemeral=True)
            else:
                await invocation_channel.send(embeds=[embed_from_payload(task_error_embed("UWU Cafe Rescan", error_text))])
            return
        ...
        try:
            ...
            if interaction is not None:
                await interaction.response.send_message(embed=embed_from_payload(task_error_embed("UWU Cafe Rescan", str(error))), ephemeral=True)
            else:
                await invocation_channel.send(embeds=[embed_from_payload(task_error_embed("UWU Cafe Rescan", str(error)))])
            return
    ...
    async def _request_shutdown(
        ...
        *,
        ...
        title = "UWU Cafe Restart" if restart else "UWU Cafe Stop"
    ...
    async def _open_contract_panel(self, interaction: discord.Interaction[Any]) -> None:
        ...
        if session is None:
            ...
            return
        embed = discord.Embed(
            title="UWU Cafe Contract Picker",
            description=(
    ...
    async def _confirm_import(self, interaction: discord.Interaction[Any]) -> None:
        ...
        if review is None or review.channel_id != interaction.channel_id or review.expires_at < utcnow():
            ...
            await interaction.response.edit_message(
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Import", "This import prompt expired. Run `!import` again."))],
                view=None,
            )
            return
        await interaction.response.defer()
        ...
        try:
            ...
            await _safe_edit_message(
                review.prompt_message,
                content="",
                embeds=[embed_from_payload(task_error_embed("UWU Cafe Import", f"Import failed for `{review.attachment_filename}`: {error}"))],
                view=None,
            )
            raise
        _cleanup_import_review_file(review)
    ...
    async def _perform_reset(
        ...
        filtered_bundle = ExportBundle(
            ...
            audit_events=[
                ...
                or (
                    event.action == "payout_adjustment_snapshot"
                    ...
                    embeds=[
                        task_warning_embed(
                            ...
                            (
                                ...
                                "This legacy calculator closed after 20 minutes of inactivity. Use `u!log <combo_count>` with proof instead."
                            ),
                        )
                    ],
                    components=[],
    ...
    def _register_app_commands(self) -> None:
        ...
        async def mechmanage(interaction: discord.Interaction[Any]) -> None:
            ...
            if not self._is_admin_user(interaction.user.id):
                await interaction.response.send_message(
                    embed=embed_from_payload(task_error_embed("UWU Cafe Access", "You are not an admin for this bot.")),
                    ephemeral=True,
                )
                return
            if not self._is_admin_messageable_channel(channel):
                await interaction.response.send_message(
                    embed=embed_from_payload(task_error_embed("UWU Cafe Access", "This admin command is not enabled in this channel.")),
                    ephemeral=True,
                )
                return
        ...
        @self.tree.command(
            ...
            description="Freeze this week's payout cycle and award Top Seller bonuses.",
            **command_kwargs,
        )
        @app_commands.guild_only()
        ...
        async def mechclean(interaction: discord.Interaction[Any]) -> None:
            ...
            if not self._is_scoped_messageable_channel(channel):
                await interaction.response.send_message(
                    embed=embed_from_payload(task_error_embed("UWU Cafe Access", "This command is not enabled in this channel.")),
                    ephemeral=True,
                )
                return
            ...
            await interaction.edit_original_response(
                embeds=[embed_from_payload(task_status_embed("UWU Cafe Clean", _clean_result_description(result)))],
            )
        ...
        async def mechcleanlog(interaction: discord.Interaction[Any]) -> None:
            ...
            await interaction.edit_original_response(
                embeds=[embed_from_payload(task_status_embed("UWU Cafe Main Clean", _clean_result_description(result)))],
            )
...
class ContractAddModal(discord.ui.Modal, title="Add or Update Contract"):
    ...
    async def on_submit(self, interaction: discord.Interaction[Any]) -> None:
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=embed_from_payload(task_error_embed("UWU Cafe Contracts", str(error))),
                ephemeral=True,
            )
            return
...
class AdjustPriceModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction[Any]) -> None:
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=embed_from_payload(task_warning_embed("UWU Cafe Adjust Prices", f"Price update failed. Nothing live was changed.\n{error}")),
                ephemeral=True,
            )
            return
...
def _is_lifecycle_status_message(message: discord.Message) -> bool:
    return any(embed.title == "UWU Cafe Status" for embed in message.embeds)


def _is_lifecycle_stats_message(message: discord.Message) -> bool:
    return any(
        embed.title in {"UWU Cafe Live Stats", "UWU Cafe Payouts"} for embed in message.embeds
    )
...
def _is_log_related_message(message: discord.Message, bot_user_id: int | None) -> bool:
    ...
    if content.startswith("Receipt `") and " saved for " in content:
        return True
    return any((embed.title or "").startswith("UWU Cafe Receipt ") for embed in message.embeds)
...
RECEIPT_CONTENT_PATTERN = re.compile(r"(?:Receipt|Log) `(?P<receipt_id>(?:BM|UWU)-[^`]+)`")
RECEIPT_TITLE_PATTERN = re.compile(r"^(?:UWU Cafe Receipt|UWU Cafe Log) (?P<receipt_id>(?:BM|UWU)-\S+)")
...
def _parse_note_command(remainder: str) -> tuple[str, str | None]:
    ...
    if not tokens:
        raise ValueError("Use `u!note <receipt_id> <note>`. Use `u!note <receipt_id> clear` to clear the note.")
...
def _is_matching_receipt_message(
    ...
    if f"Receipt `{receipt_id}`" in (message.content or ""):
        return True
    expected_title = f"UWU Cafe Receipt {receipt_id}"
...
def _reset_export_label(mode: ResetMode, action: ResetAction) -> str:
    if mode is ResetMode.ALL:
        return f"uwu-cafe-reset-all-{action.export_suffix()}"
    if mode is ResetMode.ONLY:
        return f"uwu-cafe-reset-selected-{action.export_suffix()}"
    return f"uwu-cafe-reset-except-{action.export_suffix()}"
...
def _adjustprices_embed(catalog_items: list[CatalogItem], notice: str | None = None) -> EmbedPayload:
    ...
    return task_status_embed("UWU Cafe Adjust Prices", description).field(
        "Next Step",
...
async def run_discord(config_path: Path | None = None) -> int:
    ...
    lock_path = preflight_config.storage.database_path.parent / "uwu-cafe.lock"
    pid_path = preflight_config.storage.database_path.parent / "uwu-cafe.pid"
    try:
